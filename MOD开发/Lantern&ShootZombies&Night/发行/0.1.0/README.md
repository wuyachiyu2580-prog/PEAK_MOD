...
- At nightfall, a one-time screen warning appears if cold resistance cannot work:
  - ShootZombies "Night Cold" is disabled in its config
  - Ascent level is below 5 (no night cold in base game)
- Bilingual messages (Chinese / English), auto-hidden after 15 seconds
...
- **Night cold not working?** The game's beta version 1.61.b fixed a bug where night cold never applied on Ascent 5+. If cold resistance never kicks in, check:
  1. Your game version is **1.61.b or later** (earlier versions have a bug that prevents night cold entirely)
  2. ShootZombies config has **Night Cold enabled**
  3. You are on **Ascent 5 or above** (night cold does not exist below Ascent 5)
- Cold-resistance multiplier requires `EnableWarmthReduction = true` to take effect
- Night cold warning only appears when entering nighttime; it does not repeat during the same night
