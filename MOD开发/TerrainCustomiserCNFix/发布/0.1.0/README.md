# TerrainCustomiserCN 0.1.0

TerrainCustomiserCN 是 `TerrainCustomiser` 的中文界面版本。

本 MOD 的目标是只替换 UI 显示文字，不改动原版地图数据、联机同步键、存档结构和核心玩法逻辑。
...
- 地图存档优先使用原版 `Snosz-TerrainCustomiser/Map Saves` 目录，方便中英文版本共用同一份地图。
...
- 这是 `0.1.0` 初版，可能仍有零星漏译。
- 个别术语可能需要根据游戏内实际显示继续校准。
- 为保证兼容性，部分内部英文名称会保留，不会强行翻译。
- 目前第四关存在原版相关 BUG，改动地形后可能不会生效，需要等待原作者更新修复；本 MOD 只是汉化，不修正原版地形生成逻辑。
