...
1.0.5 retains the focus on **quiet-by-default**, **structured data** and **targeted diagnostics**: profilers are off until you need them, while the network and RPC monitors preserve bounded, recent evidence for post-session analysis.
...
Built against PEAK 2.4.b. PEAKLib.ModConfig remains optional. Final in-game language-switching and host/client checks are still pending; the 1.0.4 measurements and testing notes below are historical.
...
4. Open `PEAK/BepInEx/` and look at:
   ...
   - `whysolaggy_data.csv` — every event in 87 fixed columns
...
### 2. Plugin Profiler — *opt-in* (default **off** in 1.0.4)
...
### 3. Patch Profiler — *opt-in* (default **off** in 1.0.4)
...
- **1.0.4** — two extra alarms layered on top (see also the Master-side diagnostics section):
...
- Built-in watched methods cover feeding / healing / status / death / revive / teleport / physics impulse / grab-kick-carry / item pickup-drop-throw / inventory / prefab spawn / end-game / ownership / tornado capture / campfire. They record sender, target GameObject + scene path, parsed arguments, payload bytes.
...
- **Thread-safe**: Photon callback work only adds records to a bounded queue. All dictionary/PhotonView work happens on the main thread, batched at `PumpBatchSize` (default 32/frame); when full, the oldest record is discarded.
- **Add custom watched methods** without recompiling:
  ```
  [RpcMonitor]
  ExtraWatchMethods = MyModRPC_Foo,AnotherModRPC_Bar
  ```
- The queue is bounded and the pump is capped, so a burst cannot create unbounded diagnostic memory use.

### 6. Structured Logging (CSV + JSONL) — always on

Every event — `FpsReport`, `PluginTiming`, `PatchTiming`, `SpikeFrame`, `AbuseAlert`, `RpcCall`, `PeriodicReport`, `HarmonyPatchMap`, `MethodTrace`, `FieldProbe`, `InstantiateTrace`, `OwnershipChange` — is also written to:

- `BepInEx/whysolaggy_data.csv` — 87 fixed columns, Excel/Power-BI friendly
...
**Automatic rotation**: when a file exceeds `MaxLogFileSizeMB` (default 10MB) it's renamed with a timestamp suffix (e.g. `whysolaggy_data_20260424_1530.csv`) and a fresh file is created. CSV and JSONL rotation files are independently cleaned up at a maximum of 30 files and 350MB by default.

Flushes once per second, and also at periodic reports and shutdown, so a crash loses at most the current short buffer.
...
The FieldProbe feature can **reflectively snapshot any field, parameter or return value at any method**, controlled entirely by a JSON rules file. Useful when you need to prove or disprove a specific hypothesis (e.g. "is this Cold-status change going through AddStatus or SetStatus?") without recompiling.
...
### 12. Log Verbosity — **Minimal by default** in 1.0.4

```ini
[Logging]
LogVerbosity = Minimal   # options: Minimal | Normal
```
...
### 13. Master-side Relay Diagnostics — always on (1.0.4, **beta**)
...
- `RemoteRpcTrace` remains as a compatibility enum value, but watched RPCs are written as `RpcCall` in 1.0.4.
- **Master-side `SuspectedRequester*`** — when `PhotonNetwork.Instantiate` fires on the Master, the tracer can correlate the most recent client-side request and write requester fields into the `InstantiateTrace` row.
- **`OwnershipChange`** — decodes EventCode 209 (request), 210 (transfer), and 212 (batch update). Request and transfer counts are kept separate; batch updates are audited but do not enter rate thresholds.
...
> Master-side requester and ownership attribution remains environment-dependent and should be treated as diagnostic evidence to correlate with the host session. On a client install, unavailable relay fields remain dormant.
...
| `whysolaggy_data.csv` | session | 87-column event stream, auto-rotated at `MaxLogFileSizeMB` |
...
| OwnershipRequestRateThreshold | 20 |
...
| QueueCapacity | 2048 | Maximum queued records; oldest records are discarded first |
...
> **Upgrading from an older version?** BepInEx only applies new default values when a key is missing. Existing values are preserved. Delete `com.wuyachiyu.WhySoLaggy.cfg` only if you want a clean configuration, or add the new keys manually.

---

## How to analyse the logs

The recommended workflow is **tag-first**: every event (native WhySoLaggy or any mod cooperating with it, e.g. `LanternShootZombiesNight`) now lands with a bracketed tag like `[LitSync]`, `[WARMTH_LOG]`, `[FuelMath]`, `[RPC_MON]`, `⚠ ABUSE ALERT`. Slice by tag first, only then zoom out.
...
`whysolaggy_data.csv` has 87 fixed columns. Pivot by `type`, filter by `method` / `sender`, chart `frameMs` over time. Great for multi-session comparison and for sharing one screenshot with teammates.
...
| StructuredLogger CSV+JSONL writer | ≈ 25 % | flush every second, 10MB rotation |
...
- All advanced features (Plugin/Patch Profilers, Dashboard, MethodTracer, FieldProbe) are **off by default** in 1.0.4. Flip them on only when you need them.
...
### ⚠ Testing status (1.0.4)

- The 1.0.4 client-side functionality (FPS monitor, RPC monitor, local Instantiate/Destroy/RPC rates, FieldProbe, MethodTracer, Harmony conflict scan, structured logging) is fully exercised.
...
- PhotonView Ownership audit — EventCode 209 / 210 / 212
...
- If you host a session with this build, send the resulting `whysolaggy_events.jsonl` and filter `type=OwnershipChange / InstantiateTrace / RpcCall` when comparing client and Master evidence.
