...
- DreamyAscent 从此按永久暂停/归档项目处理，不继续功能开发、问题排查、日志审查、构建部署或 TODO 执行。
- 除非用户明确说“恢复/重开 DreamyAscent”，否则后续不要主动推进 DA；旧的 2026-05-21 生成链排查只作为历史资料。
...
- 官方 `Generate Segment` 修复优先看 `DaRuntimeEditService` 当前逻辑：整段 runtime spawn 先统一 preclean，grouper 按 Early-before-Late 排序运行，`RunAll(true)` 后用手动父链识别 Late steps，按原版 Late 阶段调用 `LevelGenStep.Execute()` 并统一执行 `DeferredStepTiming.AfterCurrentGroupTiming`，最后统一 postrefresh。不要恢复外部 postfix guard、`Go()` 补跑或失败的材质 replay。
