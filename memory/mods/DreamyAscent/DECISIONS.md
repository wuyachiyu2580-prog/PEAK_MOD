...
- DreamyAscent 进入永久暂停/归档状态；默认不再做任何功能开发、问题排查、日志审查、构建部署、测试建议或 TODO 执行。
- 后续只有在用户明确说恢复/重开 DreamyAscent 时，才允许重新读取旧生成链资料并制定新范围。
...
- 1.62.a 当前反编译和实机日志表明，DreamyAscent 的官方 `Generate Segment` 不能逐个 child `PropGrouper.RunAll(true)`。这种 piecemeal 跑法会让父/子 grouper 互相清理，Snow/Roots/Jungle 的 Props 组容易从非 0 归零。当前决策是使用“segment root 范围 + selected grouper 过滤”的根级 pipeline：按当前 variant baseline 和 skip policy 选中 groupers，只清理和执行这些 groupers 下的 `LevelGenStep`，先 Early 后 deferred，再 Late 后 deferred，保留整段 runtime item/rope preclean/postrefresh。
- 如果官方 root selected-step pipeline 下 `PropSpawner.Execute()` 正常返回但生成 0，短期策略是保底恢复旧直接子物体，而不是继续盲目恢复整组 rocks 或材质 replay。2026-05-20/21 起 selected PropSpawner/Line/Sphere 会先把直接子物体移入隐藏备份根；只有“原本有子物体、执行后直接子物体为 0”的 step 才恢复，正常生成出任何子物体的 step 会丢弃备份。该机制是防止用户点击“生成本段”后地图被清空的安全网，不代表 raycast/constraint/material 根因已解决。
- Late supplement / root-level Late 执行必须手动沿 `Transform.parent` 查找最近 `PropGrouper`，不能依赖 `GetComponentInParent<PropGrouper>()`，否则 inactive 父层级下 Jungle `Pops_Plat` / `Props_Wall` 会收集到 0 个 Late step。补跑执行必须贴近原版：调用 `LevelGenStep.Execute()`，收集 `IDeferredStep`，再统一执行 `DeferredStepTiming.AfterCurrentGroupTiming`；不要用 `Go()` 作为官方 Late supplement。
