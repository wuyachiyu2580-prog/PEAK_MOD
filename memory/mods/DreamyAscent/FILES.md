...
- `DaRuntimeEditService.cs`：运行时生成、清理、分组刷新；2026-05-20 官方 `Generate Segment` 优先走 segment root selected-step pipeline，只执行 `groupersToGenerate` 导出的当前 variant baseline / skip policy 允许步骤，顺序为 selected Early steps -> `AfterCurrentGroupTiming` deferred -> selected Late steps -> deferred，整段 runtime item/rope preclean/postrefresh 和 PhotonView ID 分配仍保留。Jungle/Roots/Snow 相关修复点包括 `RunOfficialSegmentRootPipeline()`、`RunOfficialSegmentRootLevelGeneration()`、`CollectSelectedLevelGenSteps()`、`SelectedLevelGenStep` 和手动父链查找最近 `PropGrouper`；日志字段包括 `official segment root level pipeline used`、`official segment root pipeline used`、`selectedSteps`、`lateSteps`、`deferredAfterLate`。2026-05-20/21 又加入零输出保底恢复：selected `PropSpawner` / `PropSpawner_Line` / `PropSpawner_Sphere` 先备份直接子物体到隐藏 `DreamyAscent_GenerationBackup`，生成后若直接子物体为 0 则恢复旧物体；关键日志是 `backedUpSteps`、`restoredZeroedSteps`、`restored previous generated children after zero-output official generation` 和 `zero-output PropSpawner diagnostics`。生成链 catch 已改为完整异常堆栈日志，`FormatExceptionForLog()` 会展开 `TargetInvocationException`，LevelGen clear/execute/go 和 grouper validate 日志包含 hierarchy path。
