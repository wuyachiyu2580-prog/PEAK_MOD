...
- DreamyAscent 从 2026-05-24 起按永久暂停/归档处理；不再继续生成链、材质、性能、UI、诊断、构建或 TODO 工作。
- 旧的 `Generate Segment` seed/快速诊断/zero-output 相关记录保留为历史资料，不再作为活跃任务推进。
- 若未来用户明确恢复 DA，先读 `temp/2026-05-24.md` 确认暂停状态，再由用户重新指定范围。

## 2026-05-20/21 官方生成本段：零输出保底恢复

- 最新日志继续证明：没有 `levelgen-*-failed`、没有 `TargetInvocationException wrapper`，问题不是异常栈被吞，而是 `PropSpawner.Execute()` 正常返回但生成 0 个子物体。由于原生 `Execute()` 先 `Clear()` 再 `SpawnNew(false)`，失败时会先把旧物体清空。
- 已在 `DaRuntimeEditService.RunOfficialSegmentRootLevelGeneration()` 加“零输出保底恢复”：执行 selected root pipeline 前，把 selected `PropSpawner` / `PropSpawner_Line` / `PropSpawner_Sphere` 的直接子物体移到隐藏 `DreamyAscent_GenerationBackup`；执行后只有“原本有子物体、现在直接子物体为 0”的 step 会恢复旧物体，其它正常生成的 step 会销毁备份。
- 这不是最终解释材质/约束失败的根因修复，而是先防止 Jungle/Roots/Snow 被点击“生成本段”后清空。若底层仍失败，日志会出现 `restoredZeroedSteps>0` 和 `restored previous generated children after zero-output official generation`。
- 同步增加诊断：恢复前会对最多 16 个零输出 PropSpawner 打 `zero-output PropSpawner diagnostics`，包含 active 状态、位置/方向、`area`、ray 参数、layer mask、spawn 数量/概率、props、constraints/modifiers/post constraints/post behaviors，以及 5 个确定性 ray sample 的 hit path/layer/tag/material。
- 构建验证：临时输出和正式 Release 都通过 0 warnings / 0 errors；正式 DLL 已覆盖到 terrain profile，路径 `C:\Users\Administrator\AppData\Roaming\r2modmanPlus-local\PEAK\profiles\terrain\BepInEx\plugins\DreamyAscent.dll`，大小 `421376`，时间 `2026/5/21 0:24:28`。
- 下一次实机复测重点：Jungle/Roots/Snow 点击官方 `Generate Segment` 后视觉上不应再空；日志里看 `backedUpSteps`、`restoredZeroedSteps` 和 `zero-output PropSpawner diagnostics`。Beach 地形材质仍未解决，继续不要恢复已退回的材质 replay/子缩放同步方案。

## 2026-05-20 官方生成本段：Beach 物品正常，Jungle 空段二次修复

- 最新日志复查后确认：完整堆栈补丁没有触发任何 `levelgen-*-failed` 或 `TargetInvocationException wrapper`，所以当前空 Props 不是异常被吞，而是 `PropSpawner.Execute()` 正常返回但生成结果为 0。Root pipeline 已进一步收窄为只执行 `groupersToGenerate` 里导出的 selected steps，不再从 segment root 扫 `activeSteps`；新流程按 owning `PropGrouper.timing` 跑 selected Early -> deferred -> selected Late -> deferred，并在清理后、Early/Late 之间调用 `Physics.SyncTransforms()`。代码临时输出编译 0/0；正式覆盖 r2modman profile 失败仅因 PEAK 仍在运行锁住 `DreamyAscent.dll`，需关游戏后重跑 Release build。
- 最新日志复查：root-scoped pipeline 已加载，Snow/Roots 仍能看到 prop groups `children -> 0`、postrefresh `itemSpawners=0`，但没有可用的 caught exception stack。已把 `DaRuntimeEditService` 生成链相关 catch 从 `ex.Message` 改为完整 `Exception.ToString()`；反射调用的 `TargetInvocationException` 会先打印内部异常堆栈再打印 wrapper。LevelGen clear/execute/go 和 grouper validate 的 OnceWarn key/message 已加入 hierarchy path，避免同类型 `PropSpawner` 只记录第一处。Release 构建 0/0，DLL 大小 `410112`。
- 进一步复查生成代码与 JSON 后确认：运行时加载的是 `data/map-data/generated/template-snapshots.json` 打包出的新版 Snapshot V2（135 snapshots），不是源码旁边旧的 `DreamyAscent Data/template-snapshots.json`；Jungle/Roots/Snow 的 PropSpawner/DecorSpawner 步骤在 JSON 中完整存在，空段不是 JSON 缺条目。
- 最新修复将官方 `Generate Segment` 从“逐个 child grouper 调 `RunAll(true)`”改成“segment root 范围内、只执行当前 baseline/跳过策略允许的 groupers”。新流程清理选中 steps，执行 selected Early steps，统一 flush `AfterCurrentGroupTiming` deferred，再执行 selected Late steps；避免 Snow `Rocks` 这类父 grouper 单独重跑时把嵌套生成组清空，也避免 Roots/Jungle 高风险 rocks 被整组恢复。
- 新日志标记：`official segment root level pipeline used` 和 `official segment root pipeline used`。Release 构建通过 0/0，DLL 已覆盖到 terrain profile，时间 `2026/5/20 21:39:17`，大小 `409600`。下一轮重点复测 Jungle/Roots/Snow：预期 props 不再大面积 `children -> 0`，postrefresh 不再全是 `itemSpawners=0`。
...
- 后续日志证明 Late step 收集已生效，但 `Go()` 补跑语义仍不等价于原版 Late 阶段：Roots/Snow 出现 `lateSupplementSteps>0`，但大量 PropSpawner/DecorSpawner 仍 `children -> 0`。已进一步修复为 `LevelGenStep.Execute()` + `DeferredTiming.AfterCurrentGroupTiming` 统一 deferred 执行，并在日志追加 `deferredAfterGroup=<n>`。最新 Release 构建 0/0，DLL 时间 `2026/5/20 20:57:50`。
