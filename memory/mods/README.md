...
- `PlayersInfo/`：当前版本 `0.2.3`，目标 PEAK `2.4.b`；已加入三档异常图标显示、死亡/晕倒距离修复、零体力饥饿倒计时布局、统一刷新和调试日志门控，发行包已建立但关键功能仍待完整实机验收；项目直接编译输出到 `C:\Users\Administrator\AppData\Roaming\r2modmanPlus-local\PEAK\profiles\2.0.a\BepInEx\plugins`。
- `StateKeeper/`：当前版本 `0.1.0` 开发中；旧 PeakRunAnalytics 已完成开发工程、本机 DLL/配置和本机数据目录改名，发行草稿暂不更新。
