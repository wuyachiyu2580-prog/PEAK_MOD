...
- 使用分类独立于效果归因；完成动作不要求可见恢复，不因录局末尾效果窗口不足而否定使用。
- 先按epoch+归因组去重再汇总物品；所有组内观察共享分类。主/次动作不合并，时间断层/死亡中断不跨越。
- 燃料、使用比例和次数分别累计，持续消耗不虚构动作次数。推断持有人不自动作为确认使用者。
- 分数是规则等级，不是经过校准的概率；历史/缺块数据保留证据和质量边界。详情见 `research/USE_RULES_2026-09-12.md`。
