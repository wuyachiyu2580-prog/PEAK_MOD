...
- The local extra-stamina bar is below the local main stamina bar. Only the teammate extra-stamina value keeps the side placement; local values use `current/cap` without `+` and can show `0/cap` while the extra-bar state is active.
