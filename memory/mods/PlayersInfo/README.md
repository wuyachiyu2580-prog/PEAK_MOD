...
- The local extra-stamina bar is below the local main stamina bar and has no separate outer `current/cap` text; its normal and petrify/affliction values remain. Teammates retain their main stamina bars and values, but their extra-stamina graphical bar is replaced by a HUD-safe side `current/cap` value without `+`.
- Bottom-left and bottom-right anchors use a default vertical clearance of `OffsetY=138`; old exact-zero bottom offsets migrate while custom offsets remain unchanged.
...
- The 0.2.0 release notes remain historical. The 0.2.1 DLL is deployed to the current 2.0.a profile path above; in-game verification is still required for the new spectator and backpack cases.
